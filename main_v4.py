...
def build_query_anomalias(anio):
    ...
    Mantiene el resto de condiciones (including t.o_beneficiario=28388, etc.).
    """
    query = f"""
    SELECT --FORM 01
    ...
    WHERE p.t_op IN ('C41','C42')
      ...
      -- AND t.o_beneficiario = 28388
      AND p.aa_pago = {anio}
      AND p.aa_pago != EXTRACT(YEAR FROM p.fh_pago)
    GROUP BY
    ...
    WHERE p.t_op IN ('C41','C42')
      ...
      -- AND t.o_beneficiario = 28388
      AND p.aa_pago = {anio}
      AND p.aa_pago != EXTRACT(YEAR FROM p.fh_anulacion)
    GROUP BY
        CAST(p.aa_pago AS VARCHAR2(10)),
        'PAG',
        CAST(p.o_pago AS NUMBER),
...
def build_query_total(anio, beneficiarios):
    if not beneficiarios:
        # Retorna algo que no falle (p. ej. no devuelve nada).
        return "SELECT * FROM DUAL WHERE 1=0"

    # Convertimos la lista en texto "28388,12345,67890"
...
def main():
    ...
    try:
        ...
        for anio in range(anio_inicial, anio_final + 1):
            ...
            # 3.1 Construir y ejecutar Query Anomalías
            ...
            # Ordenar por BENEFICIARIO si hay registros
            ...
            df_anom.to_excel(writer, sheet_name=sheet_anom_name, index=False)
            print(f"  -> Anomalías año {anio}: {len(df_anom)} registros.")

            # 3.2 Obtener lista de beneficiarios únicos
            benef_list = df_anom["BENEFICIARIO"].unique().tolist() if not df_anom.empty else []

            # 3.3 Construir y ejecutar Query Total para esos beneficiarios
            if len(benef_list) == 0:
            ...
            df_total.to_excel(writer, sheet_name=sheet_tot_name, index=False)
            print(f"  -> Total año {anio}: {len(df_total)} registros.")
